...
let does_return_sut config value =
  let rec aux ty =
    match ty.ptyp_desc with
    | Ptyp_arrow (_, _, r) -> aux r
    | Ptyp_constr _ -> Cfg.is_sut config ty
    | _ -> false
  in
  aux value.ty
...
let arb_cmd_case config value =
  let open Reserr in
  let is_create = value.sut_vars = [] && does_return_sut config value in
...
let run_case config sut_name value =
  ...
  let* rhs =
    ...
    let pushes =
      ...
      if does_return_sut config value then
...
let next_state_case state config state_ident nb_models value =
  ...
  let open Reserr in
  let* idx, rhs =
    if value.sut_vars = [] && not (does_return_sut config value) then
              ...
              List.filter_map
                (fun (i, { model; description }) ->
                  subst_term ~out_of_scope:value.ret state ~gos_t:value.sut_vars
                    ~old_t:sut_map ~new_t:unmodified_map description
                  >>= ocaml_of_term config
                  |> to_option
...
let postcond_case config state invariants idx state_ident new_state_ident value
  ...
  let* ret_val =
    ...
    | None ->
        ...
        if does_return_sut config value then ok dummy
        else
          let* () =
            warn
          ...
          let suts =
            if does_return_sut config value then
              ...
              let ret_id =
                ...
                | _ ->
                    failwith
                      "should not happen, can only return exactly one sut"
              in
  ...
  let* rhs =
    ...
    let lhs =
      ...
      let expr =
        pexp_ifthenelse (evar last) (estring "r")
          (Some
             (if does_return_sut config value && may_raise_exception value then
                res_match
                  [%expr "Ok " ^ SUT.get_name [%e evar suts] 0]
                  (estring "_")
              else if does_return_sut config value then
    ...
    let shift =
      ...
      let expr =
        ...
        else if does_return_sut config value then eint 1
        else eint 0
      in
      value_binding ~pat ~expr
    in
